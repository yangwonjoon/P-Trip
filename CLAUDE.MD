...
코드 작성 후 아래 항목을 반드시 수행:

1. **기술 문서 업데이트** — 새로운 기술 선택, 아키텍처 결정, 외부 API 연동 등이 있었으면 `docs/TECH.md`에 반영
2. **코드 주석** — 복잡한 로직, 의도가 불분명한 코드, 비즈니스 규칙에는 "왜 이렇게 했는지" 주석 추가
3. **TODO.md 업데이트** — 완료된 태스크 ✅ 처리, 새로 발견된 작업 추가
4. **빌드 확인** — `npm run build` 통과 확인 후 커밋
